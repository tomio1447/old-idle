...
function normalizeVisualState(raw,auth){
  ...
  const sanitizeMegaIntent=(src)=>{
    ...
    const megaSteer=Number(src.steer);
    ...
    if(megaKind==="fish")
      return{kind:"fish",hold:!!src.hold,
...
function spellValues(auth,p,s){
  ...
  // Idle balance: Knight / Sorcerer +15% no dano base das magias de ataque.
  if(s&&s.type==="attack"){
    const mul=CanaryVocation.idleBaseDamageMul(p&&p.voc);
...
function takeInventoryCount(p,slug){
  ...
  // Preferir stack cheia na bag; senão instância parcial.
  if((Number(p.bag[slug])||0)>0){p.bag[slug]--;if(!p.bag[slug])delete p.bag[slug];return{ok:true,charges:accessoryCatalogCharges(slug)};}
  if(Array.isArray(p.itemInstances)){
    const idx=p.itemInstances.findIndex((inst)=>inst&&inst.loc==="bag"&&inst.slug===slug);
    if(idx>=0){
  ...
  if((Number(p.lootPouch[slug])||0)>0){p.lootPouch[slug]--;if(!p.lootPouch[slug])delete p.lootPouch[slug];return{ok:true,charges:accessoryCatalogCharges(slug)};}
  if((Number(p.supplyStash[slug])||0)>0){p.supplyStash[slug]--;if(!p.supplyStash[slug])delete p.supplyStash[slug];return{ok:true,charges:accessoryCatalogCharges(slug)};}
  if((Number(p.supplies[slug])||0)>0){p.supplies[slug]--;if(!p.supplies[slug])delete p.supplies[slug];return{ok:true,charges:accessoryCatalogCharges(slug)};}
  return false;
}
...
function maliceSlideBlocks(auth,st){
  ...
  for(const b of st.blocks||[]){
    if(b.y>=MALICE_QTE_SIZE){
      ...
      let x=1+Math.floor(random(auth)*(MALICE_QTE_SIZE-2)),guard=0;
      while(forbidden.has(x)&&guard++<40)x=1+Math.floor(random(auth)*(MALICE_QTE_SIZE-2));
      b.x=x;
    }
  }
  return hit;
...
function arenaBossSpawnTick(auth,now){
  ...
  if(now<(Number(st.at)||0))return false;
...
function initializeAuthority(descriptor,instanceId,now){
  ...
  if(descriptor.kind==="boss"){
    ...
    if(auth.bossId==="goshnar-s-megalomania"){
      ...
      const remain=combat.arenaBossSpawn&&Number(combat.arenaBossSpawn.at)>auth.clock
        ?(Number(combat.arenaBossSpawn.at)-auth.clock):bossArenaSpawnDelayMs(auth.bossId);
      arenaBossDeferSpawn(auth,boss,remain);
      if(combat.arenaBossSpawn&&Number(combat.arenaBossSpawn.at))
        auth.arenaBossSpawn.at=Number(combat.arenaBossSpawn.at);
...
module.exports={initializeAuthority,materializeAuthority,advanceAuthorityState,protectedPlayer,applyPvpLoss,expForLevel,maxStats,
  ...
  CURRENCY_GOLD,
  shareAccountGoldWallets,sellAuthAllPouch,sellAuthPouchItem,destroyAuthPouchItem,setAuthAutoSupplyStash,setAuthLootConfig,
