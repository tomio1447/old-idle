/*
 * pale_worm_lobby.js — lobby 1–9 jogadores (1 char cada) para The Pale
 * Worm, o último boss do Feast of Souls. Mesmo sistema do lobby do
 * Megalomania (convites por nome, templo obrigatório, sem party), mas com
 * 9 vagas e o requisito próprio do boss: o personagem precisa ter matado
 * os outros 3 bosses do Feast of Souls (Dread Maiden, Fear Feaster,
 * Unwelcome) para entrar no lobby.
 *
 * Arquivo separado do megalomania_lobby.js — nenhum comportamento do
 * Megalomania é alterado.
 */
"use strict";
...
function createPaleWormLobbyController(opts) {
  opts = opts || {};
  ...
  function detachAccount(accountId, optsIn) {
    ...
    // Lobby aberto: líder sair dissolve (comportamento antigo do mega).
  ...
  function closeLobby(lobby, reason) {
    ...
    for (const s of lobby.slots) {
      ...
      publishAccount(s.accountId, "pale-lobby", { action: "closed", reason: lobby.closedReason, lobby: null });
    }
  ...
  function publishLobby(lobby) {
    ...
    for (const s of lobby.slots) {
      if (!s) continue;
      publishAccount(s.accountId, "pale-lobby", {
  ...
  /* Gate do Pale Worm: matou os 3 bosses anteriores do Feast of Souls. */
  function gateStatus(character) {
    const data = parseCharData(character);
    const bosses = (data && data.bosses) || {};
    const missing = [];
    for (const id of GATE_BOSSES) {
      const st = bosses[id];
      if (!st || !(Number(st.kills) > 0)) missing.push(id);
    }
    return { ok: missing.length === 0, missing };
  }

  function gateLabel(id) {
    if (id === "the-dread-maiden") return "The Dread Maiden";
    if (id === "the-fear-feaster") return "The Fear Feaster";
    if (id === "the-unwelcome") return "The Unwelcome";
    return id;
  }

  function assertCanEnter(character) {
    const g = gateStatus(character);
    if (!g.ok) {
      return {
        ok: false,
        msg: "É preciso ter matado " +
          g.missing.map((id) => gateLabel(id)).join(", ") +
          " para entrar no lobby do Pale Worm.",
      };
    }
    return { ok: true };
  }
  ...
  async function createLobby(db, acc, character, optsIn) {
    ...
    if (party) return { code: 409, body: { ok: false, msg: "Saia da Party antes de abrir o lobby do Pale Worm." } };
    const can = assertCanEnter(character);
    ...
    const lobby = {
      ...
      slots: [slotPayload(acc.id, optsIn.playerName || character.name, character),
  ...
  async function invite(db, acc, inviteeName) {
    ...
    const can = assertCanEnter(target);
    if (!can.ok) return { code: 403, body: { ok: false, msg: `${target.name}: ${can.msg}` } };
    ...
    publishAccount(inv.toAccountId, "pale-lobby", {
  ...
  async function declineInvite(db, acc, inviteId) {
    ...
    publishAccount(inv.fromAccountId, "pale-lobby", {
   ...
   * (rota leave-fight) deve encerrar a instância. */
  async function leaveFight(db, acc) {
    try {
      ...
      publishAccount(acc.id, "pale-lobby", {
        action: "left-fight",
        lobby: null,
        msg: "Você saiu do lobby do Pale Worm.",
      });
        ...
        if (successorId && successorId !== Number(acc.id)) {
          try {
            ...
            if (tr && tr.ok && tr.instance) {
              ...
              publishAccount(successorId, "pale-lobby", {
                action: "takeover",
                lobby: lobbyPublic(result.lobby, successorId),
                instanceId: String(tr.instance.instance_id),
                msg: "Você assumiu o controle da sala do Pale Worm.",
              });
            ...
            console.error("[pale-lobby] transfer ownership failed:", e && e.message);
          }
        }
      }
      return {
      ...
      console.error("[pale-lobby] leaveFight:", e && e.message);
      return {
        code: 200,
        body: {
          ok: true, lobby: null, remaining: 0, shouldEndInstance: true,
  ...
  async function kick(db, acc, targetAccountId) {
    ...
    publishAccount(targetAccountId, "pale-lobby", { action: "kicked", lobby: null });
  ...
  async function start(db, acc) {
    ...
    // Revalida party/gate de todos no momento do start.
    ...
    for (const m of members) {
      ...
      publishAccount(m.accountId, "pale-lobby", {
        action: "start",
        members,
  ...
  function stateFor(accountId) {
    ...
    return {
      ...
      limits: { maxSlots: MAX_SLOTS, gateBosses: GATE_BOSSES.slice() },
    };
  }
  ...
  return {
    ...
    bindShare, sharedForAccount, stateFor, listInbox,
    getLobbyForAccount, getLobbyByInstanceOwner, membersForStart, closeLobby, lobbies,
    assertCanEnter, gateStatus, gateLabel,
  };
}

module.exports = {
  createPaleWormLobbyController, MAX_SLOTS, GATE_BOSSES, BOSS_ID,
};
