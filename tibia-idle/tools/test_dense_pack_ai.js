...
/* SD no isolado: forçar primary=iso ainda assim NÃO deve SD se pack existe e wave pronta */
...
/* Só 1 mob restante → single-target OK */
...
aloneItem.p._spellCd = { "exevo-flam-hur": now + 99999 };
...
const knightP = Object.assign(magePlayer({
  ...
  config: {
    ...
    combo: [
      ...
      { kind: "spell", id: "exori-gran", min: 1 }
    ]
  }
}));
...
must(!(kPick && kPick.id === "exori-ico"),
  "knight usou exori ico no isolado com pack perto: " + JSON.stringify(kPick && kPick.id));
/* Longe da box: exori-gran na posição atual pega ≤1 → não castar, deixa andar */
must(!kPick || kPick.id !== "exori-ico",
  "strike single não deveria vencer a prioridade de pack");
...
ctx.ensureCombo(pCombo);
/* Wave em CD → SD com 1 mob */
const origPronta = ctx.comboPronta;
ctx.comboPronta = (c, p, entrada, alvo, t) => {
  if (entrada && entrada.id === "exevo-flam-hur") return false;
  return origPronta(c, p, entrada, alvo, t);
};
