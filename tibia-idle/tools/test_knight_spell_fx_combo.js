...
must(html.includes("js/combat.js?v=")&&
  (html.includes("js/game.js?v=fx-sqm-align-v1")||html.includes("js/game.js?v="))&&
  html.includes("js/player.js?v=")&&
  (html.includes("js/render.js?v=fx-sqm-align-v1")||html.includes("js/render.js?v="))&&
  html.includes("js/grid.js?v="),
  "cache-bust knight-fx-combo / fx-sqm-align ausente");
const gameJs=fs.readFileSync(path.join(game,"js","game.js"),"utf8");
must(gameJs.includes("addCombatHitLog")&&gameJs.includes("isComboLead")&&
  gameJs.includes("cellToScreen(Number(cel.cx), Number(cel.cy), gw, gh)")&&
  gameJs.includes("setGridSize(gw,gh)"),
  "cliente sem combo físico/elemento, areafx no SQM absoluto ou setGridSize online");
...
must(renderJs.includes("function mergeFloaterText")&&renderJs.includes("comboKey")&&
  renderJs.includes("ef.comboKey === comboKey"),
  "combo visual Canary (mesmo alvo+elemento) ausente no floater/FX");
...
const grouped=visualCtx.aggregateCombatVisualEvents([
  ...
  {t:"hit",targetId:"shared",el:"physical",dmg:55},
...
must(physGroup&&physGroup.total===95&&physGroup.count===2&&
