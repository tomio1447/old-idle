...
// Sparky Beam (Canary CONST_ME_BLOCKHIT, length=5 spread=0): não pode cair no
// wiki Floating Block / elevador de pedra via alias block-hit→block-effect.
const assassinBeam = (ctx.GAMEDATA.monsters['cobra-assassin'].skills || [])
  .find(sk => sk.length === 5 && (sk.spread || 0) === 0);
must(assassinBeam && assassinBeam.fx === 'block-hit' && assassinBeam.el === 'physical',
  'Sparky Beam do Assassin ausente/incorreto');
...
must(!effectCtx.FX_OFFICIAL_ALIASES['yellow-rings'],
  'Yellow Rings ainda troca para Yellow Sparkles');
must(!effectCtx.FX_OFFICIAL_ALIASES['block-hit'],
  'Sparky Beam ainda troca Block Hit por Floating Block/wiki block-effect');
...
must(!effectCatalog.aliases['yellow-rings'],
  'catálogo fonte ainda regenera o alias incorreto de Yellow Rings');
must(!effectCatalog.aliases['block-hit'],
  'catálogo fonte ainda regenera o alias incorreto de Block Hit');
...
const yellowRings = fs.readFileSync(path.join(game, 'assets', 'fx', 'yellow-rings.png'));
must(yellowRings.readUInt32BE(16) === 224 && yellowRings.readUInt32BE(20) === 32,
  'strip oficial Yellow Rings deveria ter 7 quadros de 32px');
const blockHit = fs.readFileSync(path.join(game, 'assets', 'fx', 'block-hit.png'));
must(blockHit.readUInt32BE(16) === 96 && blockHit.readUInt32BE(20) === 32,
  'strip oficial Block Hit deveria ter 3 quadros de 32px');
must(indexSource.includes('<script src="js/effectdata.js?v=block-hit-fix-v1"></script>'),
  'effectdata do Block Hit / Sparky Beam está sem cache-busting');
