...
must((!live.mobs || !live.mobs.length) && live.arenaBossSpawn && live.arenaBossSpawn.pending,
  'Scarlett deveria nascer com defer da arena');
must(live.arenaBossSpawn.pending.cx===19 && live.arenaBossSpawn.pending.cy===7,
  'newBossCombat ignorou spawn da Scarlett');
must(live.arenaBossSpawn.pending.maxHp===130000,
  'newBossCombat não aplicou os 130000 HP da Scarlett');
ctx.arenaBossSpawnTick(live, Date.now() + 60000);
must(live.mobs[0] && live.mobs[0].cx===19 && live.scarlett && live.scarlett.immune,
  'Após defer, Scarlett não ligou imunidade');
...
must(scarlettSrc.includes('SCARLETT_TIMING_WINDOW = 500') &&
  ...
  scarlettSrc.includes('translate3d') &&
  scarlettSrc.includes('scarlettClearTrackCache') &&
  scarlettSrc.includes('_wallOffset'),
...
must(scarlettSrc.includes('Mantém dues wall-clock travados') ||
  scarlettSrc.includes('_wallOffset'),
  'Hydrate online ainda reamostra offset a cada tick (esteira saltava)');
must(indexSrc.includes('scarlett-boss.js?v=scarlett-esteira-smooth-v1'),
...
const layoutSrc=fs.readFileSync(path.join(game,'css','layout.css'),'utf8');
const scarlettQteCss=(layoutSrc.match(/\.scarlett-qte \{[^}]+\}/)||[])[0]||'';
must(layoutSrc.includes('will-change: transform') &&
  scarlettQteCss && !scarlettQteCss.includes('filter:'),
  'CSS da esteira ainda anima left/drop-shadow no root');

// Continuidade visual: posição da nota é função linear do tempo (sem degraus).
const due = 10000;
const p0 = ctx.scarlettNoteLeftPct({ due, hit:false }, due - 1400);
const p1 = ctx.scarlettNoteLeftPct({ due, hit:false }, due - 700);
const p2 = ctx.scarlettNoteLeftPct({ due, hit:false }, due);
must(p0 > p1 && p1 > p2 && Math.abs((p0 - p1) - (p1 - p2)) < 0.01,
  'Esteira não avança de forma contínua/linear no tempo');

// Hydrate online: mesmo QTE não pode recriar dues (salto).
const onlineFight = { boss, player:a, players:[a], authClock:5000,
  scarlett:{ phase:'qte', sequence:['up','down','left','right','up'],
    noteDues:[6000,6820,7640,8460,9280], index:0, qteStartedAt:5000, notes:[], thresholdIndex:0 } };
must(ctx.scarlettHydrateOnlineNotes(onlineFight) === true, 'Hydrate online falhou na 1ª vez');
const dues1 = onlineFight.scarlett.notes.map(n => n.due);
const wall1 = onlineFight.scarlett._wallOffset;
onlineFight.authClock = 5200; // tick seguinte
must(ctx.scarlettHydrateOnlineNotes(onlineFight) === true, 'Hydrate online falhou na 2ª vez');
must(wall1 === onlineFight.scarlett._wallOffset,
  'Offset wall-clock mudou entre ticks (causaria chop na esteira)');
must(JSON.stringify(dues1) === JSON.stringify(onlineFight.scarlett.notes.map(n => n.due)),
  'Dues wall-clock recriados no 2º hydrate');
...
console.log('OK: Scarlett, loot, outfit, magias, mapa, QTE, gates, permadeath, corpses e esteira fluida validados.');
