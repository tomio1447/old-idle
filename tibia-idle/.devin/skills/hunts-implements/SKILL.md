...
5. Para cada item de loot, confirme:
   - registro em `GAMEDATA.items` com `cid` que renderiza um ícone 1×1 no `Tibia.dat` convertido (valide com `render_item_860` e `img.getbbox()` — o `cid` do `items.xml` do Canary/15.x nem sempre é o id do cliente);
   - sprite PNG extraído corretamente em `game/assets/item/<slug>.png` (rode `tools/extract_860.py` e verifique as dimensões do PNG gerado);
