...
function tryUseRune(c, p, target, now) {
  ...
  for (const slug in p.supplies) {
    ...
    if (!s || s.type !== "attack" || p.supplies[slug] <= 0) continue;
    if (!best || s.tier > SUPPLIES[best].tier) best = slug;
  }
  if (!best) return false;
  const s = SUPPLIES[best];
  p.supplies[best]--;
  if (p.supplies[best] <= 0) delete p.supplies[best];
  c.stats.supplyUsed[best] = (c.stats.supplyUsed[best] || 0) + 1;
  c.stats.supplyCost += supplyPrice(s, p.level);
  c.runeCd = now + 2000;
...
function tryHeal(c, p, now) {
  ...
  if (p.config.useRunes) {
    ...
    for (const slug in p.supplies) {
      const s = SUPPLIES[slug];
      if (!s || s.type !== "heal" || p.supplies[slug] <= 0) continue;
      if (!best || s.tier > SUPPLIES[best].tier) best = slug;
    }
    if (best) {
      const s = SUPPLIES[best];
...
function canHeal(c, p) {
  ...
  if (p.config.useRunes) {
    for (const slug in p.supplies) {
      const s = SUPPLIES[slug];
      if (s && s.type === "heal" && p.supplies[slug] > 0) return true;
    }
  }
  return false;
}

/* Recompra automatica de supplies com o ouro da propria hunt.
 * E o que torna a caçada "auto-sustentavel" como no Baiak Idle. */
function autoRestock(p, budget) {
  if (!p.config.useRunes || !p.config.autoRestock) return 0;

  // Orcamento: nunca gasta mais do que uma fracao do ouro em caixa.
  // Isso impede que o restock seque o char e o deixe sem gold liquido.
  let purse = budget !== undefined ? budget : Math.floor(p.gold * 0.6);
  if (purse <= 0) return 0;

  // uma cura por tier; a de ataque so entra se sobrar ouro
  const heals = [], attacks = [], manas = [];
  for (const slug in SUPPLIES) {
    const s = SUPPLIES[slug];
    if ((s.lvl || 1) > p.level) continue;
    if (s.type === "heal") heals.push([slug, s]);
    else if (s.type === "attack") attacks.push([slug, s]);
    else if (s.type === "mana") manas.push([slug, s]);
  }
  const byTier = (a, b) => b[1].tier - a[1].tier;
  heals.sort(byTier); attacks.sort(byTier); manas.sort(byTier);

  const isMage = VOCATIONS[p.voc].weapon === "magic";
  // Estoque alvo cresce com o nivel: char baixo nao precisa de 150 runas.
  const healTarget = Math.min(150, 25 + p.level);
  // prioridade: cura > mana (mago) > runa de ataque
  const plan = [];
  if (heals.length) plan.push([heals[0][0], heals[0][1], healTarget, 0.6]);
  if (isMage && manas.length) plan.push([manas[0][0], manas[0][1], 100, 0.2]);
  // runas de ataque so valem a pena quando o loot paga por elas
  if (attacks.length && p.level >= 25)
    plan.push([attacks[0][0], attacks[0][1], 100, 0.2]);

  let spent = 0;
  for (const [slug, s, target, share] of plan) {
    const have = p.supplies[slug] || 0;
    if (have >= target) continue;
    const price = supplyPrice(s, p.level);
    const allowance = Math.floor(purse * share);
    const canBuy = Math.min(target - have, Math.floor(allowance / price));
    if (canBuy <= 0) continue;
    const cost = canBuy * price;
    p.gold -= cost;
    spent += cost;
    p.supplies[slug] = have + canBuy;
  }
  return spent;
}
