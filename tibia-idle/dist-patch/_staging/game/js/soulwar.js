...
function maliceSlideBlocks(st){
 ...
 // Recicla blocos que saíram do tabuleiro — nunca na coluna do start/goal/jogador.
 ...
 for(const b of st.blocks){
  if(b.y>=MALICE_QTE_SIZE){
   ...
   let x=1+Math.floor(((st.randomFn||Math.random)())*(MALICE_QTE_SIZE-2)),guard=0;
   while(forbidden.has(x)&&guard++<40)
    x=1+Math.floor(((st.randomFn||Math.random)())*(MALICE_QTE_SIZE-2));
   b.x=x;
  }
 }
 return hit;
...
function maliceRenderMinigame(c,now){
 ...
 el.innerHTML=`<div class="malice-title">GOSHNAR'S MALICE</div>
  ...
  <small>Azul→vermelho em 12s · falha = 6000 death em todos · trash respawn 20s</small>`;
}
...
function maliceRenderQte(c,now){
 ...
 el.innerHTML=`<div class="malice-qte-title">LABIRINTO DA MALÍCIA — ${left}s</div>
  ...
  <div class="malice-qte-help">Clique (passo em direção) ou WASD/setas · azul → vermelho · blocos caem</div>`;
