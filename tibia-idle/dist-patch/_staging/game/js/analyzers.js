...
function otcAnalyserItemIcon(id, amount) {
  ...
  return typeof itemImg === "function" ? itemImg(id, 18, null, amount) : "";
}
...
function otcAnalyserBody(kind, c) {
  ...
  if (kind === "loot" || kind === "supply") {
    ...
    const rows = Object.entries(source || {}).sort((a, b) => Number(b[1]) - Number(a[1])).map(([id, amount]) =>
      `<div class="otc-analyser-line">${otcAnalyserItemIcon(id, amount)}<span>${otcAnalyserItemName(id)}</span><strong>${otcAnalyserNumber(amount)}</strong></div>`
    ).join("");
    if (kind === "supply") {
