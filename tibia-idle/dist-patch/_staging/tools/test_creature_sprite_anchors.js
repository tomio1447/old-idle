...
const originEnd = renderSrc.indexOf('function effectTileOrigin', originStart);
must(originStart >= 0 && originEnd > originStart,
  'não isolou creatureTileOrigin');
const geoCtx = {};
...
must(appearanceSrc.includes('markSpriteAnchor(cv, renderMeta, 64, 64)') &&
     ...
     appearanceSrc.includes('wantMounted ? ".mounted"'),
  'renderer de outfits/mounts ignora âncora/zPattern');
