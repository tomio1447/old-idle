...
must(html.includes("js/party.js?v=")&&html.includes("js/training.js?v=party-checkpoint-v1"),
...
const liveLeader={id:"leader",hp:0,mp:0,level:10,conditions:{poison:{dmg:5,turns:3}},buffs:{haste:999999},soulWarTaints:{level:2,bosses:{}}};
const liveMember={id:"member",hp:0,mp:0,level:20,conditions:{fire:{dmg:4,turns:2}},buffs:{utito:999999},soulWarTaints:{level:1,bosses:{}}};
const rosterLeader={id:"leader",hp:0,mp:0,level:10,conditions:{energy:{dmg:3,turns:1}},buffs:{haste:999999},soulWarTaints:{level:2}};
const rosterMember={id:"member",hp:0,mp:0,level:20,conditions:{bleed:{dmg:2,turns:4}},buffs:{utito:999999},soulWarTaints:{level:1}};
const saved=[];
const ctx={
 ...
 clearPlayerCombatConditions(p){if(!p)return false;p.conditions={};return true;},
 saveCharacterToRoster(p){saved.push({id:p.id,hp:p.hp,mp:p.mp,conditions:p.conditions});},
 partyOnlineMode(){return false;},partyLocalData(){return {members:[{id:"member"}]};},
 partyLocalInvolved(c){return !!(c&&(c.id==="leader"||c.id==="member"));},
...
must(!Object.keys(liveLeader.conditions||{}).length&&!Object.keys(liveMember.conditions||{}).length&&
  !Object.keys(rosterLeader.conditions||{}).length&&!Object.keys(rosterMember.conditions||{}).length,
  "templo/restore não limpou conditions");
must(liveLeader.buffs&&liveLeader.buffs.haste&&liveLeader.soulWarTaints&&liveLeader.soulWarTaints.level===2&&
  liveMember.buffs&&liveMember.buffs.utito&&rosterMember.soulWarTaints&&rosterMember.soulWarTaints.level===1,
  "templo/restore apagou buffs ou máculas");
must(partySrc.includes("clearPlayerCombatConditions")||partySrc.includes("conditions = {}"),
  "partyCombatRestoreAll sem limpeza de conditions");
must(partySrc.includes("ent.permadead = false"),
  "partyCombatRestoreAll sem limpar permadead");
...
console.log("OK: templo, training e entrada de arena restauram HP/MP, limpam conditions e estados mortos da party (buffs/taints intactos).");
