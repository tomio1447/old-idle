...
(async () => {
  ...
  must(gameClient.includes('account.role === "admin"') &&
       ...
       !gameClient.includes('const adminAllowed = true'),
    'painel Admin deve liberar TEST_SERVER + role admin (não sempre true)');
  ...
  const forgedCoins=await post('/api/coins',{token:friendToken,amount:100});
  must(forgedCoins.status===200&&forgedCoins.data.ok&&forgedCoins.data.coins>=100,
    'TEST_SERVER deve permitir self-admin de Coins para conta comum');
  ...
  must(firstFriend.status===201&&firstFriend.data.coins===125,
  ...
  must(secondFriend.status===201&&secondFriend.data.coins===125,
    'criação de personagem adicional duplicou o bônus de TC');
  const friendLease=await post('/api/lease/acquire',{token:friendToken,holder_id:'friendholder'});
  must(friendLease.status===200&&friendLease.data.leaseToken,'lease da conta friend falhou');
  const friendCharId=firstFriend.data.character.id;
  const friendGrant=await request('/api/characters/'+friendCharId,{
    method:'PUT',headers:{'content-type':'application/json'},
    body:JSON.stringify({token:friendToken,expected_version:firstFriend.data.character.saveVersion,
      admin_grant:true,voc:'knight',level:88,
      holder_id:friendLease.data.holderId,lease_token:friendLease.data.leaseToken,
      data:JSON.stringify({id:String(friendCharId),name:'Friend One',voc:'knight',level:88,exp:1})}),
  });
  must(friendGrant.status===200&&friendGrant.data.ok&&friendGrant.data.character.level===88,
    'TEST_SERVER deve aceitar admin_grant da própria conta friend');
