...
must(combat.mega._pendingSeed&&combat.mega._pendingSeed.slug,"pendingSeed ausente");
...
// Sem pendingBoss: ainda reconstrói (não marca spawned vazio).
const combatEmpty={boss:{id:"goshnar-s-megalomania"},mobs:[],events:[],
  player:{id:"p",p:player,cx:15,cy:16,x:.5,y:.53},
  gridW:30,gridH:30,hunt:hunt,huntMap:{rows:Array(30).fill(".".repeat(30)),leg:{".":{v:[1]}},
    mob:[{x:15,y:8}]}};
combatEmpty.mega={bossSpawnAt:1000,bossSpawned:false,pendingBoss:null,_pendingSeed:null,
  phase:"waiting",immune:true,personal:{},randomFn:()=>0.1};
ctx.megaBossTick(combatEmpty,1000);
must(combatEmpty.mega.bossSpawned&&(combatEmpty.mobs||[]).some((m)=>m.boss&&m.hp>0),
  "Boss deveria reconstruir sem pendingBoss");
...
must(typeof ctrl.queueIntent==="function"&&typeof ctrl.drainIntents==="function",
  "lobby precisa enfileirar intents de convidados");

// initializeAuthority deve recuperar pendingBoss do snapshot do cliente.
const pending={id:"boss1",slug:"goshnar-s-megalomania-green",boss:true,hp:620000,maxHp:620000,
  cx:15,cy:8,x:.5,y:.27};
const descriptor=engine.initializeAuthority({
  kind:"boss",bossId:"goshnar-s-megalomania",huntId:"goshnars-megalomania-room",
  instanceMode:"boss",activeCharacterId:"1",
  members:[{id:"1",p:{id:"1",name:"Knight",voc:"knight",level:500,hp:12000,mp:500}}],
  state:{gridW:30,gridH:30,mobs:[],mega:{pendingBoss:pending,bossSpawned:false}}
},"a".repeat(64),1000);
must(descriptor.authority&&descriptor.authority.mega&&!descriptor.authority.mega.bossSpawned,
  "auth mega sem spawned no init");
must(descriptor.authority.mega.pendingBoss||descriptor.authority.mega._pendingSeed,
  "auth deve recuperar pendingBoss do cliente");
must(!(descriptor.authority.mobs||[]).some((m)=>m.boss),
  "boss fica fora de mobs até o spawn deferido");
