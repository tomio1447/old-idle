...
must(combatSrc.includes("BOSS_SPAWN_DELAY_MS")&&combatSrc.includes("arenaBossDeferSpawn")&&
  combatSrc.includes("arenaBossSpawnTick"),
  "combat.js sem delay de spawn compartilhado");
...
{
  const spawnCtx={console,Math,Date,Map,Set};vm.createContext(spawnCtx);
  ...
  must(spawnCtx.bossArenaSpawnDelayMs("goshnar-s-spite")===5000&&
    spawnCtx.bossArenaSpawnDelayMs("goshnar-s-hatred")===5000&&
    spawnCtx.bossArenaSpawnDelayMs("goshnar-s-megalomania")===15000,
    "delay de spawn Spite/Hatred/Mega divergente");
  const boss={slug:"goshnar-s-spite",id:"boss-s",boss:true,x:.5,y:.5};
  const c={mobs:[boss],events:[]};
  const t0=2_000_000;
  must(spawnCtx.arenaBossDeferSpawn(c,boss,5000,t0)===true&&c.mobs.length===0&&
    spawnCtx.arenaBossSpawnPending(c)&&!spawnCtx.arenaBossSpawnTick(c,t0+4999)&&
    spawnCtx.arenaBossSpawnTick(c,t0+5000)&&c.mobs[0]===boss&&c.arenaBossSpawn.spawned,
    "Spite: boss deve surgir só após 5s de defer");
}
...
must(authSrc.includes("const BOSS_SPAWN_DELAY_MS=5000")&&
  ...
  "servidor sem delay 5s compartilhado");
