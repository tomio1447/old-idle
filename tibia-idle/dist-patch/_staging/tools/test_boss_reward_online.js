...
function silence(auth){
  const pending=auth.authority.arenaBossSpawn&&auth.authority.arenaBossSpawn.pending;
  const mobs=[...(auth.authority.mobs||[])];
  if(pending)mobs.push(pending);
  for(const mob of mobs){mob.damage=0;mob.attackSpeed=Number.MAX_SAFE_INTEGER;mob.attackAcc=0;}
  for(const item of auth.authority.players||[]){item.p.hp=engine.maxStats(item.p).hp;item.p.conditions={};}
}
function liveBoss(auth){
  return (auth.authority.mobs||[]).find((m)=>m.boss)||
    (auth.authority.arenaBossSpawn&&auth.authority.arenaBossSpawn.pending)||null;
}
...
let greed=engine.initializeAuthority(greedDesc,"b".repeat(64),1000);
silence(greed);
must(!greed.authority.greed&&liveBoss(greed),
  "Greed não deve ligar mecânica antes do spawn deferido");
greed=waitArenaBoss(greed,1000);
silence(greed);
const boss=greed.authority.mobs.find((m)=>m.boss);
must(boss&&greed.authority.greed&&greed.authority.greed.immune,
  "Greed não surgiu imune após o delay de arena");
...
const afterGreed=JSON.parse(engine.advanceAuthorityState(JSON.stringify(greed),2000,
  Number(greed.authority.clock)+2000).state);
...
let hatred=engine.initializeAuthority(bossDesc(player(),"goshnar-s-hatred"),"c".repeat(64),1000);
silence(hatred);
must(!hatred.authority.hatred&&hatred.authority.arenaBossSpawn&&hatred.authority.arenaBossSpawn.pending,
  "Hatred não deve agendar Dread's Torment antes do spawn");
hatred=waitArenaBoss(hatred,1000);
silence(hatred);
must(hatred.authority.hatred&&!hatred.authority.hatred.active&&
  hatred.authority.hatred.nextActivationAt>=hatred.authority.clock+20000&&
  hatred.authority.hatred.nextActivationAt<=hatred.authority.clock+40000,
  "Hatred não agendou Dread's Torment em 20–40s após o spawn");
...
const scarlettInit=engine.initializeAuthority(bossDesc(player(),"scarlett-etzel"),"d".repeat(64),1000);
silence(scarlettInit);
must(!scarlettInit.authority.scarlett&&liveBoss(scarlettInit),
  "Scarlett não deve ligar QTE antes do spawn deferido");
const scarlett=waitArenaBoss(scarlettInit,1000);
silence(scarlett);
const scarlettBoss=scarlett.authority.mobs.find((m)=>m.boss);
must(scarlettBoss,"Scarlett não surgiu após o delay de arena");
...
must(scarlett.authority.scarlett&&scarlett.authority.scarlett.immune&&scarlettBoss.qteImmune,
  "Scarlett não ficou imune após o spawn");
const stillImmune=JSON.parse(engine.advanceAuthorityState(JSON.stringify(scarlett),2000,
  Number(scarlett.authority.clock)+2000).state);
